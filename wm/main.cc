...
static void keyboard_handle_key(
	...
	if (event->state == WL_KEYBOARD_KEY_STATE_PRESSED) {
		for (int i = 0; i < nsyms; i++) {
			...
			} else if (syms[i] == XKB_KEY_Kanji || syms[i] == XKB_KEY_Eisu_toggle) {
				if (auto p = server->input_method) if (auto q = server->focused_text_input) {
					p->on ^= true;
					if (p->on) {
						wlr_input_method_v2_send_activate(*p);
						q->send();
	...
	if (!handled) if (auto p = server->input_method) if (auto q = p->input_method->keyboard_grab) {
		wlr_input_method_keyboard_grab_v2_set_keyboard(q, keyboard->wlr_keyboard);
...
static void server_new_keyboard(struct tinywl_server *server,
	...
	auto keyboard = static_cast<tinywl_keyboard*>(calloc(1, sizeof(tinywl_keyboard)));
...
static struct tinywl_toplevel *desktop_toplevel_at(
	...
	*surface = scene_surface->surface;
	...
	return tree ? static_cast<tinywl_toplevel*>(tree->node.data) : NULL;
}
...
static void server_new_output(struct wl_listener *listener, void *data) {
	...
	auto output = static_cast<tinywl_output*>(calloc(1, sizeof(tinywl_output)));
...
static void server_new_xdg_toplevel(struct wl_listener *listener, void *data) {
	...
	auto toplevel = static_cast<tinywl_toplevel*>(calloc(1, sizeof(tinywl_toplevel)));
...
static void server_new_xdg_popup(struct wl_listener *listener, void *data) {
	...
	auto popup = static_cast<tinywl_popup*>(calloc(1, sizeof(tinywl_popup)));
...
int main(int argc, char *argv[]) {
	...
	server.input_method_input_method.notify = [](auto a_listener, auto a_data)
	{
	...
	wl_signal_add(&server.input_method_manager->events.input_method, &server.input_method_input_method);
	server.text_input_manager = wlr_text_input_manager_v3_create(server.wl_display);
	server.text_input_text_input.notify = [](auto a_listener, auto a_data)
	{
	...
	wl_signal_add(&server.text_input_manager->events.text_input, &server.text_input_text_input);
	server.keyboard_state_focus_change.notify = [](auto a_listener, auto a_data)
	{
	...
	wl_signal_add(&server.seat->keyboard_state.events.focus_change, &server.keyboard_state_focus_change);
