...
void t_engine::f_roman_reset()
{
	...
	f_on_compose(i, n, nullptr, nullptr, 0);
}
...
void t_engine::f_text_pop()
{
	if (v_states.back().v_text > 0) {
		...
		v_buffer.pop_back();
		--state.v_text;
		f_on_compose(state.v_text, 1, nullptr, nullptr, 0);
	} else if (v_states.size() > 1) {
		v_states.pop_back();
		t_state& state = v_states.back();
...
void t_engine::f_entry_pop()
{
	...
	if (state.v_entry > 0) {
		v_buffer.pop_back();
		--state.v_entry;
		f_on_compose(state.v_text + state.v_entry, 1, nullptr, nullptr, 0);
	}
	if (state.v_entry <= 0) {
		v_mode = &t_engine::f_mode_roman;
		f_on_status();
...
void t_engine::f_choose()
{
	...
	if (v_candidates.empty()) {
		...
		f_on_compose(0, n, nullptr, nullptr, 0);
	} else {
...
void t_engine::f_mode_latin(xkb_keysym_t a_key, char a_ascii)
{
	if (a_key == XKB_KEY_Return || a_ascii == 'M' - '@') {
		f_text_commit();
	} else if (a_key == XKB_KEY_Linefeed || a_ascii == 'J' - '@') {
		v_mode = &t_engine::f_mode_roman;
		f_on_status();
	} else if (a_key == XKB_KEY_BackSpace || a_ascii == 'H' - '@') {
		f_text_pop();
...
void t_engine::f_mode_word(xkb_keysym_t a_key, char a_ascii)
{
	if (a_key == XKB_KEY_Return || a_ascii == 'M' - '@') {
		f_entry_commit();
	} else if (a_key == XKB_KEY_Linefeed || a_ascii == 'J' - '@') {
...
void t_engine::f_mode_roman(xkb_keysym_t a_key, char a_ascii)
{
	if (a_key == XKB_KEY_Return || a_ascii == 'M' - '@') {
...
void t_engine::f_mode_kanji(xkb_keysym_t a_key, char a_ascii)
{
	if (a_key == XKB_KEY_Return || a_ascii == 'M' - '@') {
		if (v_map != &t_roman_table::v_default) f_roman_reset();
		f_entry_commit();
...
void t_engine::f_mode_choose(xkb_keysym_t a_key, char a_ascii)
{
	t_state& state = v_states.back();
	size_t n = v_candidates[v_chosen].v_text.size();
	if (a_key == XKB_KEY_BackSpace || a_ascii == 'H' - '@' || a_ascii == 'x' && v_chosen <= 0) {
		if (state.v_okuri > 0) n += state.v_okuri - 1;
		v_mode = &t_engine::f_mode_kanji;
		v_buffer.erase(v_buffer.begin() + state.v_base + state.v_text + state.v_entry, v_buffer.end());
		state.v_okuri = 0;
		...
		f_on_status();
		f_on_candidates();
		if (a_key == XKB_KEY_Return || a_ascii == 'M' - '@') return;
...
std::wstring t_engine::f_entry(size_t a_i) const
{
	...
	auto p = &v_buffer[state.v_base + state.v_text];
	std::vector cs(p, p + state.v_entry);
	if (state.v_okuri > 0) {
		cs.push_back(L' ');
		p += state.v_entry;
