	...
	v_host.v_surface.v_on_key_press = v_host.v_surface.v_on_key_repeat = [&](auto a_key, auto a_c)
	{
		...
		if (code != t_code::c_NONE) {
			...
			if (code == t_code::c_INSERT) {
				if (i == 1) {
					if (auto p = f_client().f_selection(); p && p->f_mime_types().contains("text/plain")) p->f_receive("text/plain", [&, utf8 = std::array<char, 1024>(), utf8n = size_t(0), out = [&](auto a_p, auto a_n)
					{
						f_send(a_p, a_n);
					}](auto a_fd) mutable
					{
						...
						utf8n += n;
						auto p = utf8.data();
						auto e = v_utf8tomb.f_to<char>(&p, &utf8n, out);
						if (e == EINVAL && n > 0) {
							std::copy_n(p, utf8n, utf8.data());
							e = 0;
						}
						if (e == 0 && n == 0) e = v_utf8tomb.f_to<char>(nullptr, nullptr, out);
