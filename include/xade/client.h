...
public:
	...
	void f_receive(const char* a_mime_type, auto&& a_read)
	{
		int fds[2];
		...
		suisha::f_loop().f_poll(fds[0], POLLIN, [read = std::move(a_read), fd = fds[0]](auto a_events) mutable
		{
...
{
	wl_data_device_set_selection(v_data_device, a_source.release()->v_source, v_action_serial);
}
